...
def main(text):
		#Creating project like html,css and java, etc
		if "project" in text:
		...
		if "create" in text and "file" in text:
			...
			return
		#translate the sentence in any language
		if "translate" in text:
			...
			return
		#creating schedule list for you
		if 'list' in text:
			...
			if isContain(text, ['show', 'my list']):
				...
				speak(items[0])
				return
		#showing battery and your system information
		if isContain(text, ['battery', 'system info']):
			...
			return
		#Telling meaning of any sentence
		...
		#automating open video for you on youtube
		...
		if isContain(text, ['search', 'image']):
			...
			return
		#showing you the direction of your giving path
		...
		if isContain(text, ['weather']):
			...
			speak(data[-1])
			return
		#taking screenshot for your current screen
		...
		#if command are not in our feature then it redirect to google search
...
if __name__ == '__main__':
	splash_root = Tk()
	splash_root.configure(bg='#2b2e30')
	splash_root.overrideredirect(True)
	splash_label = Label(splash_root, text="Processing...", font=('montserrat',15),bg='#2b2e30',fg='white')
	...
	#User Photo
	userProfileImg = Image.open("userData/faceData/img4.png")
